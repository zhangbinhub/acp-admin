<template>
  <Card>
    <Form ref="searchForm" :model="searchForm" :label-width="80" :inline="true" class="search-form"
      ...
      <Row>
        ...
        <Form-item :label="$t('forms.startDate')" prop="startTime">
          <i-date-picker type="date" :disabled="modal_loading" :options="datePickerOptions"
        ...
        <Form-item :label="$t('forms.endDate')" prop="endTime">
          <i-date-picker type="date" :disabled="modal_loading" :options="datePickerOptions" v-model="searchForm.endTime"
    ...
    <Table border height="388" size="small" :columns="columns" :data="searchData" class="search-table"
      ...
      <template slot-scope="{ row }" slot="responseTime">
        <span>{{formatDate(row.responseTime)}}</span>
      </template>
    ...
    computed: {
      columns () {
        const columns = [
          ...
          {
            key: 'responseTime',
            title: this.$i18n.t('forms.responseTime'),
            width: 150,
            slot: 'responseTime',
            sortable: 'custom'
          },
